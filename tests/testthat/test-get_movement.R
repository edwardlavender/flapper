...
test_that("get_mvt_mobility_from_archival() throws warnings as expected", {
  ...
  arc <- data.frame(depth = c(10, 100, 10, 100, 1000)*-1,
...
test_that("get_mvt_mobility_from_archival() calculations are correct", {
  arc <- data.frame(depth = c(10, 100, 5000),
  ...
  min(est$speed_ms); mean(est$speed_ms); max(est$speed_ms)
  min(est$speed_mstep); mean(est$speed_mstep); max(est$speed_mstep)
