...
#'
#' @details This function implements routines provided via \code{\link[flapper]{flapper}} and the \code{cppRouting} package to calculate least-cost distances. The main steps are:
#' \enumerate{
...
lcp_from_point <- function(origin,
  ...
  if (!is.null(destination)) {
    if (inherits(destination, "matrix")) {
      ...
      stop("class(destination) is un-supported. Only matrix (coordinates), integer (cell IDs) or functions are supported as inputs.")
    }
  }
  ...
  function(origin,
    ...
    if (check) {
      ...
      if (method == "cppRouting") {
        if (combination == "pair") {
          cppRouting_algorithm <- check_value(
            input = cppRouting_algorithm,
    ...
    if (method == "gdistance") {
      ...
      if (goal %in% c(2, 3)) {
        ...
        if (any(!(connections %in% path_lcp_cells$path))) {
          ...
          warning(
            paste0(
              "LCPs could not be resolved for ",
...
lcp_comp <- function(surface,
  ...
  if (length(dots) > 0L) {
    ...
    if (!(all(dots_bool))) {
      ...
      stop(
        paste0(
          "The following argument(s) passed via ... are not supported: ",
        ...
        points_for_dist_by_row <-
          lapply(split(start_xy, seq_len(nrow(start_xy))), function(start_xyi) {
            ...
            if (nrow(end_xyi) == 0L) {
              ...
              points_for_dist_for_row <- data.frame(
                dist_sim = numeric(),
  ...
  cat_to_console(paste0(
    "... ... Calculating shortest-distances via cppRouting::get_distance_pair() for ",
  ...
  if (inherits(dist_lcp, "error")) {
    ...
    point_samples$dist_lcp <- dist_lcp
  }
