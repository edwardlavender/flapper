...
#'
#' @param archival A dataframe of depth time series (for a single individual). At a minimum, this should contain a column named `depth' with depth observations. Depth should be recorded using absolute values in the same units as the bathymetry (\code{bathy}, see below). Observations are assumed to have been made at regular time intervals. Other columns can be included for variables which influence the movement model (see \code{calc_movement_pr}).
...
#'     \item \strong{Distance calculations}. This step is particularly slow because the distance from each location to many or all surrounding locations are calculated. To speed up this step, implement \code{calc_distance = "euclid"} and, if necessary, interpolate least-cost paths after algorithm completion via \code{\link[flapper]{lcp_interp}}. In the future, a Markov chain Monte Carlo style approach may be implemented in which distances (and probabilities) for randomly selected `proposal' cells are calculated, with those cells then rejected or retained, rather than calculating distances to many or all surrounding cells, but this is unlikely to be faster in many settings.
#'     \item \strong{Parallelisation.} Test alternative options for parallelisation. For the \code{cl} argument, specifying an integer on non-Windows platforms may be faster than a cluster from \code{\link[parallel]{makeCluster}} (see \code{\link[pbapply]{pblapply}}). Parallelisation may be slower in some circumstances.
#'   }
#' }
#'
#' @examples
...
#'
#' #### Next steps
#' # Use dcpf_plot_history() to visualise particle histories
#' # Use dcpf_simplify() to process movement paths
#' # ... Use dcpf_loglik() to examine the relative likelihood of alternative paths
#' # ... Use dcpf_plot_1d(), dcpf_plot_2d(), dcpf_plot_3d() for plotting paths
#'
...
dcpf <- function(archival,
  ...
  for(t in update_history_from_time_step:nrow(archival)){
    ...
    if(t <= (nrow(archival) - 1)) {
      ...
      arc_t_next <- archival[t, , drop = FALSE]
...
#' @description This function is designed to simplify the \code{\link[flapper]{.dcpf-class}} object from \code{\link[flapper]{dcpf}} that defines sampled particle histories into a set of movement paths.
#' @param record A \code{\link[flapper]{.dcpf-class}} object from \code{\link[flapper]{dcpf}}.
#' @param verbose A logical input that defines whether or not to print messages to the console to monitor function progress.
#'
#' @details If there are vast numbers of possible paths through the landscape captured by particle movements, the function can run into vector memory limitations when assembling paths.
#'
#' @return The function returns a \code{\link[flapper]{dcpf-class}} object, which is a dataframe that defines the movement paths.
#'
#' @examples
#' # An example with the example DCPF particle histories dataset included in flapper
#' paths <- dcpf_simplify(dat_dcpf_histories)
#'
#' @author Edward Lavender
#' @export

dcpf_simplify <- function(record, verbose = TRUE){
