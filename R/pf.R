...
#' @param data (optional) A dataframe, with one row for each time step (i.e. element in \code{record}), and columns for any time-specific variables included in the movement models (see \code{calc_movement_pr} and \code{calc_movement_pr_from_origin}). If un-supplied, movement probabilities are constant through time.
...
#' \itemize{
#'   \item \strong{ACPF}. The ACPF algorithm combines the AC algorithm with particle filtering. This is designed to simulate possible movement paths in passive acoustic telemetry arrays. This algorithm is widely applicable.
#'   \item \strong{DCPF}. The DCPF algorithm combines the DC algorithm with particle filtering. This is designed to simulate possible movement paths of a benthic animal over the seabed, given a regular sequence of depth observations (\code{archival}), the bathymetry (\code{bathy}) over which movement occurred and a movement model (\code{calc_movement_pr}) that specifies the probability of movement from a given location to any other, given the distance between them (and any other pre-defined time-dependent parameters in \code{archival}). The function was motivated by small scale applications concerning the reconstruction of possible movement paths of flapper skate (\emph{Dipturus intermedius}) tagged with archival tags, following capture and release in a given location, for short-periods of time post-release.
#'   \item \strong{ACDCPF}. The ACDCPF algorithm combines the ACDC algorithm with particle filtering. For benthic animals with acoustic and archival data, this algorithm is designed to reconstruct fine-scale movement patterns across an area.
#'   }
#' }
#'
...
#' record <- dc_out$spatial
#' record <- lapply(record, function(r) r$map_timestep)
#' # Plot maps
...
#' history_dcpf <- pf(record = lapply(dc_out$spatial, function(r) r$map_timestep),
#'                    origin = origin_sim,
#'                    calc_distance = "euclid",
#'                    mobility = 200,
#'                    n = 10L)
...
pf <- function(record,
  ...
  ## Check data and define data_1 and data_t_next if un-supplied
  if(!is.null(data)){
