...
#' @description The ACDC algorithm requires a list of acoustic centroids from \code{\link[flapper]{acdc_setup_centroids}}. The number of centroids that is required depends on (a) the duration between detections; (b) the distances among receivers; (c) the area of interest; and (d) other considerations such as the spatial extent and resolution of bathymetry data that is required by the ACDC algorithm and presence of a long tail of archival observations after the final acoustic detection. This function implements two approaches to make suggestions as to suitable numbers of centroids for the ACDC algorithm (see Details). This requires a time series of detections, the deployment details of passive acoustic telemetry receivers and a mobility parameter. Given these inputs, the function returns a suggested upper and lower bound for the number of centroids.
#'
...
#'
#' @seealso This function is designed to facilitate an informed choice for the 'n_timesteps' argument in \code{\link[flapper]{acdc_setup_centroids}}. This underpins the ACDC algorithm, which is implemented by \code{\link[flapper]{acdc}}.
#'
#' @examples
#' n_timesteps <-
...
#' @param n_timesteps An integer that defines the the number of timesteps after a hypothetical detection for which centroids will be created, where the duration of each timestep is given by the duration between archival observations (see \code{\link[flapper]{acdc_setup_n_centroids}}).
