...
The AC/DC branch functions (`ac()`, `dc()` and `acdc()`) all return objects of class `acdc`. These can be processed and analysed using several key functions: 
...
* `acdc_helper_*()` functions provide simple 'helper' routines for working with simplified outputs: 
    ...
    * `acdc_helper_list_record_for_pf()` creates an ordered list of output files (that can be passed to a particle filtering routine (see below);
